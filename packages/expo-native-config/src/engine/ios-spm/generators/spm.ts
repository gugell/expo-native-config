...
function findTarget(project: XcodeProject, name?: string): PBXNativeTarget {
  if (name) {
    ...
    if (!match) {
      throw new Error(`[expo-native-config] swiftPackages: target "${name}" not found.`);
    }
    return match as PBXNativeTarget;
  }
  const app = project.rootObject.getMainAppTarget('ios');
  if (!app) {
    throw new Error('[expo-native-config] swiftPackages: no main app target found.');
  }
  return app;
}
  ...
  return withMeta(
    {
      ...
      base: 'ios',
      path: 'Podfile',
      tag: `expo-native-config-spm-pod-target-${spec.slug}`,
