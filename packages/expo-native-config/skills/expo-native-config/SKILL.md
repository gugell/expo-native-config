...
description: Configure native iOS targets, dependencies, schemes, and Android settings in Expo apps that use expo-native-config.
---
...
The config is `workspace.config.ts`, default-exporting `defineWorkspace({ schemaVersion: 1, ios: { ... }, android: { ... } })`. Import helpers from `expo-native-config`. Register `expo-native-config/plugin` in the existing Expo `plugins` array while preserving other entries.
...
1. **Expo's own config** — `app.json` owns identity, host `infoPlist`, entitlements, `intentFilters`, `permissions` and `blockedPermissions`, `googleServicesFile`. This package has no `android.permissions` field on purpose; `<uses-feature>` and `queries`, which Expo's config does not cover, are here. `expo-build-properties` owns `useFrameworks`, host `deploymentTarget`, `packagingOptions`, ProGuard. Do not duplicate those here.
...
4. **Escape hatches** — `ios.podfile` (`postInstall`, `lines`, `replace`) and `android.gradle` replace rules rewrite generated Ruby/Groovy. Expo's guidance treats regex rewrites of generated code as a last resort; they break silently across SDK upgrades. Every one is planned with `risk: "escape-hatch"` and warned about by `doctor`. Set `required: true` so a rule that stops matching fails the build. Re-verify them after every SDK upgrade.

This package never edits `AppDelegate` or `MainApplication`. Startup work belongs in the hooks Expo provides — an `ExpoAppDelegateSubscriber` on iOS, a `ReactActivityLifecycleListener` or application lifecycle listener on Android — implemented in a local Expo module and configured through `android.strings`. `init --template lifecycle-module` scaffolds that module (listener, subscriber, string resource and the config entry) and is the answer to "how do I run code at launch", which is the documented channel for values native code reads before the JS engine starts. Do not propose editing a generated entry point.
...
```ts
import { defineWorkspace, Scheme, Target } from 'expo-native-config';
...
Discovery checks `workspace.config.ts`, `.js`, `.cjs`, `.mjs`, and `.json` in order. For a custom path, pair CLI `--config config/native.json` with Expo plugin registration `['expo-native-config/plugin', { configPath: 'config/native.json' }]`. Keep other plugins and append this registration once. JSON uses ordinary objects, such as `{ name: 'Share', type: 'share' }`, instead of helper calls.
